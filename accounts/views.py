...
def register_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            # authenticate to attach backend when multiple authentication backends are configured
            authed_user = authenticate(username=user.email, password=form.cleaned_data.get("password1"))
            login(request, authed_user or user, backend='accounts.backends.EmailOrUsernameModelBackend')
